...
SLIDES = {
    ...
    2: {
        ...
        "key_insight": "DGAT1 overexpression is a validated cancer survival mechanism — yet no DGAT1 inhibitor exists in oncology. This is BrownBioTech's opening.",
    },
    3: {
        "type": "solution",
        "title": "Our Solution",
        ...
        "highlight": "In vivo Confirmed  ·  First-in-Class  ·  IND Q3 2026",
    },
    4: {
        "type": "market",
    ...
    5: {
        ...
        "bullets": [
            ...
            "In vivo PK/PD modeling reduces late-stage attrition",
            "Platform applicable beyond DGAT1 — YARS2 program in early discovery",
        ],
        "platform_highlight": "YARS2: Second program, early discovery (aminoacyl-tRNA synthetase)",
    },
    6: {
        "type": "pipeline",
        "title": "Pipeline",
    ...
    7: {
        ...
        "advisors": [
            {"name": "KOL #1 (Being Onboarded)", "role": "Clinical Oncology Advisor", "affiliation": "Top US Cancer Center"},
            {"name": "KOL #2 (Being Onboarded)", "role": "Computational Biology Advisor", "affiliation": "Leading AI/ML in Pharma"},
        ],
        "highlights": [
...
/* ── Title slide ── */
.slide-title {
  background: linear-gradient(135deg, #2A0F05 0%, #4A2010 50%, #2A0F05 100%);
  color: white;
...
.slide-title .slide-header { border-bottom-color: rgba(255,255,255,0.15); }
...
.pipeline-table tr:nth-child(odd) td { background: rgba(74,32,16,0.03); }
.lead-badge {
  background: var(--accent);
  color: white;
  font-size: 11px;
...
.founder-card .detail strong { opacity: 1; }
...
.funds-table td:nth-child(2) { font-weight: 700; color: var(--brown); }
.funds-table tr:nth-child(odd) td { background: rgba(74,32,16,0.03); }
...
<div class="deck-nav">
  <a href="#" id="nav-prev">&#8592;</a>
  <a href="#" id="nav-next">&#8594;</a>
</div>
...
(function() {
  ...
  document.getElementById('nav-prev').onclick = function(e) {
    e.preventDefault();
    deck.scrollBy({ left: -w, behavior: 'smooth' });
  };
...
class PitchDeckGenerator:
    ...
    def generate_slide(self, slide_num: int) -> str:
        ...
        return generators.get(data["type"], self._slide_generic)(data)

    def generate_full_deck(self) -> dict[int, str]:
        """Return all slides as dict of {slide_num: markdown}."""
...
### {d['platform_highlight']}"""

    def _slide_pipeline(self, d: dict) -> str:
        rows = []
        for p in d["programs"]:
            badge = '<span class="lead-badge">Lead</span>' if p["highlight"] else ""
            rows.append(f"| {p['name']} | {p['target']} | {p['indication']} | {p['stage']} | {p['milestone']} {badge} |")
        table = "\n".join(rows)
        return f"""\
## {d['title']}

### BROWN-1 (DGAT1)
- **Status:** {d['programs'][0]['status']} | **Stage:** {d['programs'][0]['stage']}
- **Next Milestone:** {d['programs'][0]['milestone']}

### BROWN-2 (YARS2)
- **Status:** {d['programs'][1]['status']} | **Stage:** {d['programs'][1]['stage']}
- **Next Milestone:** {d['programs'][1]['milestone']}
...
> **{d['vision']}**"""
    ...
    def _render_html_slide(self, num: int, content: str) -> str:
        """Convert markdown content to styled HTML for a single slide."""
        import re
        ...
        def flush():
            ...
            if bullet_buffer:
                body_parts.append(
                    "<ul class=\"bullets\">"
                    + "".join(f"<li>{b}</li>" for b in bullet_buffer)
                    + "</ul>"
                )
            bullet_buffer = []
        ...
        return f"""\
<div class="slide slide-{slide_type}" id="slide-{num}">
  <div class="slide-header">
    <span class="slide-num">{num} / 8</span>
  ...
  <div class="slide-content">
    ...
    def _add_pptx_slide(self, prs: Presentation, num: int, content: str):
        ...
        # Title text box
        txb = slide.shapes.add_textbox(Inches(0.5), Inches(0.3), Inches(12.33), Inches(1.0))
        tf = txb.text_frame
        tf.paragraphs[0].text = title
        tf.paragraphs[0].font.size = Pt(36)
        tf.paragraphs[0].font.bold = True
        tf.paragraphs[0].font.color.rgb = RGBColor(0x4A, 0x20, 0x10)

        # Subtitle
        if subtitle:
            txb2 = slide.shapes.add_textbox(Inches(0.5), Inches(1.2), Inches(12.33), Inches(0.6))
            tf2 = txb2.text_frame
            tf2.paragraphs[0].text = subtitle
            tf2.paragraphs[0].font.size = Pt(20)
            tf2.paragraphs[0].font.color.rgb = RGBColor(0xC4, 0x77, 0x3A)

        # Collect bullets and other content
        ...
        if bullets:
            start_y = 2.0 if subtitle else 1.6
            txb3 = slide.shapes.add_textbox(Inches(0.5), Inches(start_y), Inches(12.33), Inches(5.0))
            tf3 = txb3.text_frame
            tf3.word_wrap = True
